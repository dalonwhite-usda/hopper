...
def main() -> None:
    ...
    ap.add_argument("--solver", type=str, default="glpk", help="glpk|cbc|gurobi|cplex")
    args = ap.parse_args()

    # Inputs
    forage_params = load_forage_in(args.root / "FORAGE.IN")
    # Weather assumption: column 3 in CURRENT.WTR is Tavg (deg F)
    ...
    # Base monthly forage pre-treatment; rescaled per treatment to match annual abs yield
    base_monthly = monthly_forage_lbs_per_acre(tmean_c, forage_params, annual_abs_lbs=yield_info["base"])

    # Parameters (assumptions + report values)
    treated_acres = 16044
    brood_cows = 480
    req_lb_per_cow_month = 780.0  # assumption: approx 1 AUM/month
    hay_buy_cost_per_ton = 80.0  # report
    grass_hay_inventory_lbs = 300 * 1.5 * 2000.0  # report -> 900,000 lbs
    allowed_months = ["MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"]
    blm_total_aum = 723  # report
    state_total_aum = 138  # report
    price_per_aum = 1.92  # report
    lb_per_aum = 780.0  # assumption
    ...
    # Solve baseline (No Treatment) first
